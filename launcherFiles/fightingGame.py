...
def fightingGameMain():
    ...
    def initSound():
        ...
        Player.hitSounds = [
            pygame.mixer.Sound(os.path.join(SOUND_PATH, "soundeffect2.wav")),
        ...
        pygame.mixer.music.load(
            os.path.join(filepath, "fightinggame.wav")
        )  # must be wav 16bit and stuff?
    ...
    class Platform:
        ...
        def draw(self):
            pygame.draw.rect(
                ...
                (127, 127, 127),
                (
                    self.hurtboxes[0] + shakeX,
                    self.hurtboxes[1] + shakeY,
                    self.hurtboxes[2] - self.hurtboxes[0],
                    self.hurtboxes[3] - self.hurtboxes[1],
                ),
                0,
            )
    ...
    class Projectile:
        ...
        def confirmedHit(self, damage):
            pass
            if (
                not isinstance(self.owner, Alien)
                and not isinstance(self.owner, Animals)
                and not isinstance(self.owner, Darkgreen)
            ):
        ...
        def draw(self):
            ...
            if self.op:
                gameDisplay.blit(
                    image[self.facingRight],
                    (
                        int(self.x + random.randint(-1, 1) * 8 + shakeX),
                        int(self.y + random.randint(-1, 1) * 8 + shakeY),
                    ),
                )
            else:
                gameDisplay.blit(
                    image[self.facingRight], (int(self.x + shakeX), int(self.y + shakeY))
                )
            ...
            if random.random() < 0.9 and DEBUG_MODE:
                if self.hitboxes:
                    pygame.draw.rect(
                        ...
                        (255, 255, 0),
                        (
                            self.hitboxes[0] + shakeX,
                            self.hitboxes[1] + shakeY,
                            self.hitboxes[2] - self.hitboxes[0],
                            self.hitboxes[3] - self.hitboxes[1],
                        ),
                        0,
                    )
        ...
        def physics(self):
            ...
            data = self.box
            self.hitboxes = [
                self.x + data[0] * Player.SCALE,
                self.y + data[1] * Player.SCALE,
                self.x + data[2] * Player.SCALE,
                self.y + data[3] * Player.SCALE,
                data[4],
            ]
            if len(data) > 5:
            ...
            for player in Player.players + Projectile.projectiles + Platform.platforms:
                ...
                if self.hurtboxes[2] > otherBox[0] and self.hurtboxes[0] < otherBox[2]:
                    if self.hurtboxes[3] > otherBox[1] and self.hurtboxes[1] < otherBox[3]:
                        if self in Projectile.projectiles and not (
                            isinstance(player, Player) and player.invincible
                        ):
                            Projectile.projectiles.remove(self)
    ...
    class Player:
        ...
        def getJoystickControls(stick):
            ...
            return Player.getControls("xbox","xbox")
            ...
            if self.random:
                ...
                for i in ["1", "2", "3", "4"]:
                    ...
                    if self.pressed[i]:
                        ...
                        if isinstance(self, Alien) and (
                            self.pressed["3"] or self.pressed["4"]
                        ):
                            self.pressed["d"] = target.x < self.x
                        if (
                            isinstance(self, Lizard) or isinstance(self, Penguin)
                        ) and self.pressed["4"]:
                            self.pressed["d"] = target.x < self.x
                            # self.pressed["w"] = True
        ...
        def physics(self):
            ...
            # hit
            if (
                not self.stun or self.stun > 16 and self.attackFrame > 10
                ...
                for player in Player.players + Projectile.projectiles:
                    if (
                        ...
                        and not (
                            ...
                            and isinstance(player.owner, Sad)
                            and player.lifetime < 20
                        )
                    ):
                        ...
                        if self.collide(enemybox):
                            if len(player.hitboxes) == 8:
                                self.hurt(
                                    player,
                                    player.hitboxes[4],
                                    knockback=player.hitboxes[5],
                                    stun=player.hitboxes[6],
                                    angle=player.hitboxes[7],
                                )
                                ...
                                self.hurt(
                                    player, player.hitboxes[4], knockback=player.hitboxes[5]
                                )  # player/player.owner for ult charge. bad for pos
                            else:
                                self.hurt(
                                    player, player.hitboxes[4]
                                )  # .owner for ult charge. bad for pos
                            if isinstance(player, Projectile):
                                Projectile.projectiles.remove(player)

        def hurt(
            self, player, damage, knockback=None, stun=None, angle=0
        ):  #  angle: [-1 to 1]
            if self.invincible:
            ...
            if knockback:
                if isinstance(self, Glitch):
                    ...
                    self.x += (
                        16 * knockback * (self.facingRight - 0.5) * (-0.2 + angle * 0.2)
                    )
                    self.xv = 0
            ...
            if damage > 10 + random.random() * 40:
                ...
                gameDisplay.blit(
                    theImage, (int(self.x) + self.facingRight * 20, int(self.y))
                )
        ...
        def collide(self, otherBox):
            ...
            if (
                self.hurtboxes[2] > otherBox[0] + glitchConstant
                and self.hurtboxes[0] < otherBox[2] - glitchConstant
            ):
                if (
                    self.hurtboxes[3] > otherBox[1] + glitchConstant
                    and self.hurtboxes[1] < otherBox[3] - glitchConstant
                ):
                    return True
            return False
        ...
        def load(playerName, textureName, skin=0):
            image = pygame.image.load(
                os.path.join(filepath, "textures", playerName, textureName)
            )
        ...
        def draw(self):
            factor = 0.3
            leftEdge = int(
                (self.hurtboxes[0] + self.hurtboxes[2] - self.maxhp * factor) * 0.5 + shakeX
            )
            if not self.invisible and self.ultCharge > 1:
                if self.ultCharge >= self.CHARGE:
                    color = (
                        255,
                        255 - 255 / 16 * self.ultError,
                        255 - 255 / 16 * self.ultError,
                    )
                elif self.ultError:
                    color = (
                        127 + 128 / 16 * self.ultError,
                        255 - 255 / 16 * self.ultError,
                        255 - 255 / 16 * self.ultError,
                    )
                else:
                ...
                wdt = int(
                    min(
                        (self.maxhp * factor + 8),
                        ((self.maxhp * factor + 8) * self.ultCharge) // self.CHARGE,
                    )
                )
                pygame.draw.rect(
                    gameDisplay,
                    color,
                    (leftEdge - 4, int(self.hurtboxes[1] - 32 - 4 + shakeY), wdt, 16),
                    0,
                )
            if self.hp > 0 and not self.invisible:  # health bars
                pygame.draw.rect(
                    ...
                    (
                        leftEdge,
                        int(self.hurtboxes[1] - 32 + 1 + shakeY),
                        int(self.maxhp * factor),
                        6,
                    ),
                    0,
                )
                pygame.draw.rect(
                    ...
                    (
                        leftEdge,
                        int(self.hurtboxes[1] - 32 + shakeY),
                        int(self.hp * factor),
                        8,
                    ),
                    0,
        ...
        def __init__(self, x, y, facingRight, controls, joystick=None, skin=0):
            ...
            self.long = [
                ...
                [
                    100,
                    self.punchImage,
                    [32 - 9 - 7, 32 - 8 - 6, 32 - 9, 32 - 8, 45, 50, 55],
                ],
    ...
    class Darkpuncher(Player):
        def __init__(self, x, y, facingRight, controls, joystick=None, skin=0):
            ...
            self.long = [
                ...
                [
                    70,
                    self.punchImage,
                    [32 - 9 - 7, 32 - 8 - 6, 32 - 9, 32 - 8, 5, 40, 5, -0.8],
                ],
                [
                    75,
                    self.longPunchImage,
                    [32 - 7, 32 - 8 - 6, 32, 32 - 8, 30, 40, 40, 0.5],
                ],
            ...
            self.extreme = [
                ...
                [
                    70,
                    self.punchImage,
                    [32 - 9 - 7, 32 - 8 - 6, 32 - 9, 32 - 8, 60, 60, 60, -0.5],
                ],
                [77, self.punchImage],
        ...
        def loadImages(self, skinChoice):
            ...
            self.longPunchImage = Player.load(
                "darkpuncher", "longpunch.png", skin=skinChoice
            )
        ...
        def doAttack(self, pressed):
            ...
            if self.attack == "grow":
                ...
                if self.attackFrame < 10:
                    ...
                    self.invisible = True
                    if (self.pressed["d"] - self.pressed["a"]) * (
                        self.facingRight - 0.5
                    ) > 0:
    ...
    class Animals(Player):
        ...
        def loadImages(self, skinChoice):
            ...
            self.projbImage = [
                self.projbImage[0].convert_alpha(),
                self.projbImage[1].convert_alpha(),
            ]
    ...
    class Pufferfish(Player):
        ...
        def __init__(self, x, y, facingRight, controls, joystick=None, skin=0):
            ...
            self.box = [
                ...
                16 + 4,
                32 - 6,
            ]  # 16+-3 is actual box but it grows so...
            self.maxhp = 200
            self.init2()
        ...
        def loadImages(self, skinChoice):
            ...
            self.longPunchImage = Player.load(
                "pufferfish", "longpunch.png", skin=skinChoice
            )
        ...
        def __init__(self, x, y, facingRight, controls, joystick=None, skin=0):
            ...
            self.sting = [
                ...
                [
                    10,
                    self.punchImage,
                    [24, 12, 28, 16, 14, 50, 14],
                ],  # y goes down to 16 to hit bird
                [15, self.punchImage],
    ...
    class Ninja(Player):
        ...
        def doAttack(self, pressed):
            ...
            if self.attack == "spin":
                ...
                if self.attackFrame < 13:
                    ...
                    self.yv -= 0.9
                    self.image = [self.whirl1Image, self.whirl2Image][
                        self.attackFrame % 6 < 4
                    ]
            ...
            if self.attack == "ult":
                ...
                if self.attackFrame < 10:
                    ...
                    if self.pressed["d"]:
                        self.facingRight = True
                    self.hp -= 0.2
                    self.image = [self.magicImages[2], self.magicImages[3]][
                        (self.attackFrame // 4) % 2
                    ]
        ...
        def loadImages(self, skinChoice):
            ...
            self.pierceImages = [
                Player.load("ninja", "pierce" + str(i) + ".png", skin=skinChoice)
                for i in range(1, 6)
            ]
            self.kickImages = [
                Player.load("ninja", "kick" + str(i) + ".png", skin=skinChoice)
                for i in range(1, 4)
            ]
            self.rollImages = [
                Player.load("ninja", "roll" + str(i) + ".png", skin=skinChoice)
                for i in range(1, 7)
            ]
            self.lanceImages = [
                Player.load("ninja", "lance" + str(i) + ".png", skin=skinChoice)
                for i in range(1, 5)
            ]
            self.magicImages = [
                Player.load("ninja", "magic" + str(i) + ".png", skin=skinChoice)
                for i in range(1, 5)
            ]
            ...
            self.cssImages = [
                Player.load("ninja", "standing.png", skin=i) for i in range(6)
            ]
    ...
    class Bird(Player):
        ...
        def passive(self):
            ...
            if self.pressed["w"] and (
                self.yv >= 0 or (-18 < self.yv < -17 and not self.stun)
            ):  # float
                self.yv = 1
                self.image = self.idleImage
        ...
        def __init__(self, x, y, facingRight, controls, joystick=None, skin=0):
            ...
            self.second = [
                ...
                [
                    33,
                    self.punchImage,
                    [32 - 9, 32 - 8 - 7, 32 - 1, 32 - 10, 23, 23, 23, 0.5],
                ],
                [40, self.punchImage],
    ...
    class Lizard(Player):
        ...
        def __init__(self, x, y, facingRight, controls, joystick=None, skin=0):
            ...
            self.first = [
                ...
                [
                    9,
                    self.punchImage,
                    [32 - 9 - 6, 32 - 8 - 5, 32 - 9, 32 - 8, 23, 23, 23, 0.5],
                ],
                [17, self.punchImage],
            ...
            self.second = [
                ...
                [
                    20,
                    self.punchImage,
                    [32 - 9 - 6, 32 - 8 - 5, 32 - 9, 32 - 8, 30, 30, 40, 0.5],
                ],
                [29, self.punchImage],
        ...
        def loadImages(self, skinChoice):
            ...
            self.crouchMagicImage = Player.load(
                "lizard", "magiccrouch.png", skin=skinChoice
            )
    ...
    class Skugg(Player):
        ...
        def doAttack(self, pressed):
            ...
            if self.attack == "ult":
                ...
                if self.attackFrame < 5 * 17:
                    ...
                    if self in Player.players:
                        Player.players.remove(self)
                    new = random.choice(legalClasses)(
                        self.x, self.y, self.facingRight, self.controls, self.joystick
                    )
        ...
        def loadImages(self, skinChoice):
            ...
            self.walkImages = [
                Player.load("skugg", "SkuggVarg_0" + str(i) + ".png", skin=skinChoice)
                for i in range(2, 10)
            ]
            self.elImages = [
                Player.load("skugg", "SkuggVarg_" + str(i) + ".png", skin=skinChoice)
    ...
    class Penguin(Player):
            ...
            if self.attack == "ult":
                ...
                if self.attackFrame < 96:
                    self.image = random.choice(
                        [self.haloImage, self.halo2Image, self.wizardImage]
                    )
                    self.xv = (self.facingRight - 0.5) * 8
                    self.yv -= 1
                    self.yv *= 0.95
        ...
        def loadImages(self, skinChoice):
            ...
            self.idleImage = self.ninjaImage
            self.stunnedNinjaImage = Player.load(
                "penguin", "stunnedninja.png", skin=skinChoice
            )
            ...
            self.stunnedWizardImage = Player.load(
                "penguin", "stunnedwizard.png", skin=skinChoice
            )
        ...
        def doAttack(self, pressed):
            ...
            if self.attack == "long":
                ...
                if self.attackFrame == 17:  # 15
                    if (self.x < 200 - 128 and not self.facingRight) or (
                        self.x > 800 - 128 and self.facingRight
                    ):
    ...
    def restart():
        ...
        SELECT_CONTROLS = [
            [
                Player.getControls("wasd", "iop"),
                Player.getControls("wasd", "xcv"),
                Player.getControls("wasd", "iop"), #AI
                None, #off
            ],
            [
                Player.getControls("arrow", "xcv"),
                Player.getControls("arrow", "iop"),
                Player.getControls("wasd", "iop"), #AI
                None,
            ],
        ]
        ...
        while not State.jump_out and not pressed[pygame.K_q]:
            keyboard_controls = [
                c
                for c in [
                    SELECT_CONTROLS[0][Player.AIoption],
                    SELECT_CONTROLS[1][Player.AI2option],
                ]
                if c is not None
            ]
            ...
            for event in pygame.event.get():
                ...
                # handle controller input
                    ...
                    # find the controller that did it
            ...
            if State.playerCount >= 2 or 1: # no epilepsy trailer shenanigans ig
                ...
                if actions["4"]:
                    skinChoice = (skinChoice - 1) % 6
                    lag += 0.1
                if actions["3"] or pressed[pygame.K_r]:
            ...
            pygame.draw.rect(
                gameDisplay,
                (200, 200, 200),
                (400 + 64, 0, 16 * 8, HEIGHT_FOR_CHARACTER_DISPLAY + 28 * 8),
                0,
            )
            ...
            for i in [-4, -3, -2, -1, 0, 1, 2, 3, 4]:
                gameDisplay.blit(
                    allClasses[(num - i) % len(allClasses)].cssImages[
                        skinChoice * (i == 0)
                    ][1],
                    (400 - 104 * i, HEIGHT_FOR_CHARACTER_DISPLAY),
                )
            ...
            helpTexts = [
                helpText.replace("Attack1", helpLetters[0])
                .replace("Attack2", helpLetters[1])
                .replace("Attack3", helpLetters[2])
                .replace("Down", helpLetterDown)
                for helpText in helpTexts
            ]
            # name = "<    "+name+"    >"
            textsurface = myfont.render(name, True, (0, 0, 0))
            textsurfaceChooseCharacters = myfont.render(
                "CHOOSE CHARACTERS!", True, (150, 150, 150)
            )
            textsurface2 = myfont2.render(text, True, (0, 0, 0))
            textsurfacesHelp = [
                myfont2.render(helpText, True, (0, 0, 0)) for helpText in helpTexts
            ]
            textsurfaceControls = myfont2.render("CONTROLS:", True, (0, 0, 0))
            textsurfaceAI = myfont2.render(
                "player 1: "
                + ["WASD, IOP", "WASD, XCV", "AI", "Off"][Player.AIoption]
                + "                        (press 1)",
                True,
                (0, 0, 0),
            )
            textsurfaceAI2 = myfont2.render(
                "player 2: "
                + ["Arrowkeys, XCV", "Arrowkeys, IOP", "AI", "Off"][Player.AI2option]
                + "                (press 2)",
                True,
                (0, 0, 0),
            )
            ...
            gameDisplay.blit(
                textsurface, (545 - len(name) * 24, 250 + HEIGHT_FOR_CHARACTER_DISPLAY)
            )
            ...
            if pressed[pygame.K_c] or pressed[pygame.K_j]:
                for i in range(10):
                    gameDisplay.blit(
                        Cat.cssImages[0][0],
                        (random.randint(-200, 900), random.randint(-200, 500)),
                    )
                    gameDisplay.blit(
                        Bird.eggImage[0],
                        (random.randint(-200, 900), random.randint(-200, 500)),
    ...
    pygame.display.set_icon(
        pygame.image.load(os.path.join(filepath, "textures", "puncher", "idle.png"))
    )
    Platform.generate()
    ...
    while not State.jump_out:
        ...
        if Player.freezeTime > 0:
            ...
            if Player.freezeTime <= 0:
                Player.killScreen = 0
        if len(Player.players) < 2 and not Player.freezeTime or pressed[pygame.K_ESCAPE]:
            ...
            if Player.AI2option != 3:
                if Player.AI2option == 0:
                    choices[humansBefore](
                        600,
                        300,
                        False,
                        Player.getControls("arrow", "xcv"),
                        skin=skinChoices[humansBefore],
                    )
                else:
                    choices[humansBefore](
                        600,
...
if __name__=="__main__":
    filepath=os.path.join("fightingGameFiles")
    fightingGameMain()
    pygame.quit() #?
    quit() #?
else:
    filepath=os.path.join("launcherFiles","fightingGameFiles")
    import sys
