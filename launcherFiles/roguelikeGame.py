...
def roguelikeGameMain():
    ...
    gameDisplay.blit(
        pygame.transform.scale(
            pygame.image.load(os.path.join(filepath, "textures", "loading.png")),
            display,
        ),
        (0, 0),
    )
    ...
    pygame.display.set_icon(
        pygame.image.load(
            os.path.join(filepath, "textures", "player", "warrior", "player.png")
        )
    )
    pygame.font.init()
    ...
    def blitRotate(surf, image, pos, originPos, angle):
        ...
        origin = (
            int(pos[0] - originPos[0] + min_box[0] - pivot_move[0]),
    ...
    def loadTexture(name, w, h=None, mirror=False):
        ...
        image = pygame.image.load(
            os.path.join(filepath, "textures", name)
        ).convert_alpha()
    ...
    class Floor:
        ...
        def __init__(self,presets):
            ...
            if (
                game.depth == 5
            ):  # boss rooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooom!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
            ...
            for i in range(numOfRooms):
                ...
                while roomPos in self.roomPosList:
                    ...
                    while not None in connectedRoom.links:
                        connectedRoom = random.choice(self.rooms)
                    connectionDirection = random.choice(
                        [i for i in range(4) if connectedRoom.links[i] == None]
                    )
                    roomPos = list(
                        map(
                            sum,
                            zip(
                                connectedRoom.floorPos[:],
                                directionHash[connectionDirection],
                            ),
                        )
                    )  # Vector additon of two lists of integers #lol gotta style right
        ...
        def drawMinimap(self):
            for room in self.rooms:
                ...
                pygame.draw.rect(
                    gameDisplay,
                    (200, 200, 200),
                    [display[0] - 100 + pos[0] * 20, 100 + pos[1] * 20, 10, 10],
                    0,
                )
                for link in room.links:
                    if link:
                        ...
                        pygame.draw.line(
                            gameDisplay,
                            (200, 200, 200),
                            [display[0] - 100 + pos[0] * 20 + 5, 100 + pos[1] * 20 + 5],
                            [
                                display[0] - 100 + pos2[0] * 20 + 5,
                                100 + pos2[1] * 20 + 5,
                            ],
                            3,
                        )
    ...
    class Room:
        ...
        def loadRoom(self):
            if not self.alreadyLoaded:
                if self.links[0]:
                    self.walls.append(
                        Wall(
                            self.roomSize[0] / 4 - self.doorWidth,
                            0,
                            self.roomSize[0] / 2 - self.doorWidth * 2,
                            self.wallWidth,
                        )
                    )
                    self.walls.append(
                        Wall(
                            self.roomSize[0] * 3 / 4 + self.doorWidth,
                            0,
                            self.roomSize[0] / 2 - self.doorWidth * 2,
                            self.wallWidth,
                        )
                    ...
                    self.walls.append(
                        Wall(self.roomSize[0] / 2, 0, self.roomSize[0], self.wallWidth)
                    )
                if self.links[1]:
                    self.walls.append(
                        Wall(
                            self.roomSize[0],
                            self.roomSize[1] / 4 - self.doorWidth,
                            self.wallWidth,
                            self.roomSize[1] / 2 - self.doorWidth * 2,
                        )
                    )
                    self.walls.append(
                        Wall(
                            self.roomSize[0],
                            self.roomSize[1] * 3 / 4 + self.doorWidth,
                            self.wallWidth,
                            self.roomSize[1] / 2 - self.doorWidth * 2,
                        )
                    )
                ...
                if self.links[3]:
                    ...
                    self.walls.append(
                        Wall(0, self.roomSize[1] / 2, self.wallWidth, self.roomSize[1])
                    )
        ...
        def draw(self):
            roomCorner = (
                (display[0] - self.roomSize[0]) / 2,
                (display[1] - self.roomSize[1]) / 2,
            )
            pygame.draw.rect(
                gameDisplay, [100, 80, 50], [*roomCorner, *self.roomSize]
    ...
    class Game:
        ...
        def draw(self):
            ...
            pygame.draw.rect(
                gameDisplay,
                color,
                (0, 0, (display[0] - game.room.roomSize[0]) / 2, display[1]),
            )
            pygame.draw.rect(
                gameDisplay,
                color,
                (0, 0, display[0], (display[1] - game.room.roomSize[1]) / 2),
            )
            pygame.draw.rect(
                gameDisplay,
                color,
                (
                    (display[0] + game.room.roomSize[0]) / 2,
                    0,
                    (display[0] - game.room.roomSize[0]) / 2,
                    display[1],
                ),
            ...
            pygame.draw.rect(
                ...
                (
                    0,
                    (display[1] + game.room.roomSize[1]) / 2,
                    display[0],
                    (display[1] - game.room.roomSize[1]) / 2,
                ),
            )
    ...
    class Wall:
        ...
        def adjust(self, other):  # , proj=0):
            ...
            if self.x - dx < other.x < self.x + dx:
                if self.y - dy < other.y < self.y + dy:  # is inside?
                    if (
                        other.x - (self.x - dx) < (self.x + dx) - other.x
                    ):  # find closest side
        ...
        def draw(self):
            pos = (
                int((display[0] - game.room.roomSize[0]) / 2 + self.x),
                int((display[1] - game.room.roomSize[1]) / 2 + self.y),
            )
            pygame.draw.rect(
                gameDisplay,
                (30, 30, 30),
    ...
    class Player:
        ...
        def update(self):
            ...
            if self.state == 0:
                ...
                self.basicMove()
                if (
                    pressed[pygame.K_SPACE] or pressed[pygame.K_j]
                ) and not self.invisibility:
                    self.state = 1
                    self.stateTimer = 0
        ...
        def basicMove(self, spdMult=1):
            ...
            pressed = pygame.key.get_pressed()
            dx = (pressed[pygame.K_d] or pressed[pygame.K_RIGHT]) - (
                pressed[pygame.K_a] or pressed[pygame.K_LEFT]
            )
            dy = (pressed[pygame.K_s] or pressed[pygame.K_DOWN]) - (
        ...
        def applyAttackEffects(self, targets):
            ...
            if len(targets) > 0:
                ...
                for i in range(self.shockLink):
                    ...
                    targets = [
                        target for target in targets if (target not in alreadyHit)
                    ]
                    if len(targets) > 0:
                        oldPos = (
                            (display[0] - game.room.roomSize[0]) / 2 + target.x,
                            (display[1] - game.room.roomSize[1]) / 2 + target.y,
                        )
                        target = random.choice(targets)
                        newPos = (
                            (display[0] - game.room.roomSize[0]) / 2 + target.x,
        ...
        def hurt(self, damage=1):
            ...
            if damage > 0.1:
                ...
                self.stateTimer = 0
                if self.iceBody:
                    targets = game.findEnemies(
                        self.x, self.y, self.radius * self.iceBody * 2
                    )
                    for target in targets:
                        target.freeze()
        ...
        def draw(self):
            ...
            pos = (
                int((display[0] - game.room.roomSize[0]) / 2 + self.x + shakeX),
                int((display[1] - game.room.roomSize[1]) / 2 + self.y + shakeY),
            )
            ...
            if self.burning > 0:
                gameDisplay.blit(
                    self.burningImage, (int(pos[0]) - 128 // 2, int(pos[1]) - 128 // 2)
                )
            for i in range(self.carpet):
                gameDisplay.blit(
                    Carpet.images[random.randint(0, 1)],
                    (
                        int(pos[0]) - Carpet.imageSize // 2,
                        int(pos[1] + 16 + 8 * (self.carpet - i - 1))
                        - Carpet.imageSize // 2,
                    ),
                )
            if self.freezeDamage:
                gameDisplay.blit(
                    ColdCore.image,
                    (
                        int(pos[0]) - ColdCore.imageSize // 2,
                        int(pos[1]) - ColdCore.imageSize // 2,
                    ),
                )
            if self.stateTimer % 2 == 0 or not (
                self.invincibility or self.invisibility
            ):
                gameDisplay.blit(
                    self.image[self.imageDir],
                    (pos[0] - self.imageSize // 2, pos[1] - 8 - self.imageSize // 2),
                )
            if self.iceBody:
                gameDisplay.blit(
                    IceShield.image,
                    (
                        int(pos[0] + 8 - 16 * self.imageDir) - IceShield.imageSize // 2,
                        int(pos[1] + 8) - IceShield.imageSize // 2,
                    ),
                )
            if JesterHat in self.shownItems:
                gameDisplay.blit(
                    JesterHat.image,
                    (
                        int(pos[0]) - JesterHat.imageSize // 2,
                        int(pos[1] - 32) - JesterHat.imageSize // 2,
                    ),
                )
        ...
        def drawPlayerUI(self):
            ...
            if self.hp > 0:
                pygame.draw.rect(
                    gameDisplay,
                    (0, 200, 0),
                    (30, 30, 150 * (self.hp / self.maxhp) ** 1.1, 30),
                )
            # items
            ...
            for i in range(len(self.shownItems)):
                ...
                for j in range(number_of_copies):
                    gameDisplay.blit(
                        clss.image,
                        (
                            50 + item_horizontal_spacing * j - clss.imageSize / 2,
                            100 + item_vertical_spacing * i - clss.imageSize / 2,
                        ),
                    )
                ...
                if self.library > 0:  # (self.shownItems[clss]>0):
                    ...
                    gameDisplay.blit(
                        textsurface,
                        (
                            90 + j * item_horizontal_spacing,
                            90 + item_vertical_spacing * i,
                        ),
                    )
    ...
    class Warrior(Player):
        ...
        description = [
            "The Warriors are sword-wielding brawlers who often venture into the dungeon.",
            "",
            "Abilities:",
        ...
        walkImages = [
            loadTexture("player/warrior/walk1.png", imageSize, mirror=True),
            loadTexture("player/warrior/walk2.png", imageSize, mirror=True),
        ]
        attackImages = [
            loadTexture("player/warrior/strike1.png", imageSize, mirror=True),
            loadTexture("player/warrior/strike2.png", imageSize, mirror=True),
        ]
        rollImages = [
            loadTexture("player/warrior/roll1.png", imageSize, mirror=True),
            loadTexture("player/warrior/roll2.png", imageSize, mirror=True),
            loadTexture("player/warrior/roll3.png", imageSize, mirror=True),
        ]
        ...
        def update(self):
            ...
            if self.state == 2:
                ...
                if self.stateTimer < 20:
                    ...
                    if self.fanRoll:
                        targets = game.findEnemies(
                            self.x, self.y, self.radius * self.fanRoll
                        )
                        for target in targets:
                            target.x -= self.xdir * self.fanRoll
                            target.y -= self.ydir * self.fanRoll
                        targets = game.findProjectiles(
                            self.x, self.y, self.radius * self.fanRoll
                        )
                        for target in targets:
            ...
            if self.state == 1:
                if 15 <= self.stateTimer <= 20:
                    ...
                    blitRotate(
                        ...
                        image,
                        (x, y),
                        (self.imageSize // 2, self.imageSize // 2),
                        math.atan2(-self.ydir, -self.xdir),
                    )
            # SPIN
            if self.state == 3:
    ...
    class Ranger(Player):
        ...
        description = [
            "The Rangers use rifles to keep the distance to their foes.",
            "",
            "Abilities:",
            " J : Fire the rifle.",
            " K : Reload the rifle.",
        ...
        walkImages = [
            loadTexture("player/ranger/walk1.png", imageSize, mirror=True),
            loadTexture("player/ranger/walk2.png", imageSize, mirror=True),
        ]
        attackImages = [
            loadTexture("player/ranger/fire.png", imageSize, mirror=True),
            loadTexture("player/ranger/reload0.png", imageSize, mirror=True),
        ]
        reloadImages = [
            loadTexture("player/ranger/reload1.png", imageSize, mirror=True),
            loadTexture("player/ranger/reload2.png", imageSize, mirror=True),
        ]
        ...
        def update(self):
            ...
            if self.state == 1:
                ...
                if self.stateTimer == 0:
                    if self.ammo > 0:
                        ...
                        self.image = self.attackImages[0]
                        if self.fireSword:
                            game.room.projectiles.append(
                                FireBullet(self.x, self.y, self.xdir * 8, self.ydir * 8)
                            )
                        else:
                            game.room.projectiles.append(
                                Bullet(self.x, self.y, self.xdir * 8, self.ydir * 8)
                            )
                        self.ammo -= 1
                    else:
                        self.state = 0
            ...
            if self.state == 2:
                ...
                if self.stateTimer < 10:
                    ...
                    if (
                        not (pressed[pygame.K_LSHIFT] or pressed[pygame.K_k])
                    ) and self.fastReload:
                        self.state = 0
                        self.fastReload = False
                    self.image = self.idleImage
            ...
            if self.state == 3:
                ...
                if self.stateTimer < 20:
                    ...
                    for target in targets:
                        hyp = math.sqrt(
                            (target.x - self.x) ** 2 + (target.y - self.y) ** 2 + 1
                        )  # add 1 for fun #also to not divide by 0
        ...
        def draw(self):
            ...
            if random.random() < 0.5 and self.ammo and self.state == 0:
                pygame.draw.line(
                    ...
                    (255, 0, 0),
                    pos,
                    (pos[0] + self.xdir * 500, pos[1] + self.ydir * 500),
                    1,
                )
            super().draw()
    ...
    class Thief(Player):
        ...
        description = [
            "The Thieves use stealth to avoid combat, and pick-pocket money from enemies without killing them.",
            "",
            "Abilities:",
        ...
        walkImages = [
            loadTexture("player/thief/walk1.png", imageSize, mirror=True),
            loadTexture("player/thief/walk2.png", imageSize, mirror=True),
        ]
        attackImages = [
            loadTexture("player/thief/attack1.png", imageSize, mirror=True),
            loadTexture("player/thief/attack2.png", imageSize, mirror=True),
        ]
        hideImages = [loadTexture("player/thief/hide.png", imageSize, mirror=True)]
        stealImages = [
            loadTexture("player/thief/steal1.png", imageSize, mirror=True),
            loadTexture("player/thief/steal2.png", imageSize, mirror=True),
        ]
            ...
            if self.state == 3:
                ...
                if self.stateTimer < 6:
                    ...
                    for target in targets:
                        if target.coins:
                            target.coins -= 1
                            game.room.items.append(
                                Coin(
                                    target.x + random.randint(-8, 8),
                                    target.y + random.randint(-8, 8),
                                )
                            )
    ...
    class Mage(Player):
        ...
        description = [
            "The Mages use powerful magic to overcome their foes.",
            "",
            "Abilities:",
        ...
        walkImages = [
            loadTexture("player/mage/walk1.png", imageSize, mirror=True),
            loadTexture("player/mage/walk2.png", imageSize, mirror=True),
        ]
        attackImages = [
            loadTexture("player/mage/attack1.png", imageSize, mirror=True),
            loadTexture("player/mage/attack2.png", imageSize, mirror=True),
        ]
        tpImages = [
            loadTexture("player/mage/teleport1.png", imageSize, mirror=True),
            loadTexture("player/mage/teleport2.png", imageSize, mirror=True),
        ]
        magicImages = [
            loadTexture("player/mage/magic.png", imageSize, mirror=True),
            loadTexture("player/mage/magic2.png", imageSize, mirror=True),
        ]
        ...
        def drawPlayerUI(self):
            ...
            if self.MANAMECHANIC:
                w = 150
                pygame.draw.rect(
                    gameDisplay, (0, 100, 100), ((display[0] - w) // 2, 30, w, 30)
                )
                pygame.draw.rect(
                    gameDisplay,
                    (0, 200, 200),
                    ((display[0] - w) // 2, 30, w * (self.mana / self.maxmana), 30),
                )

    def load_save_file():
        try:
            with open(
                os.path.join("roguelikeGameFiles", "savefile.txt"), "r"
            ) as save_file:
                for i in range(4):
    ...
    class Ally:
        ...
        def draw(self):
            ...
            gameDisplay.blit(
                self.image[self.facing],
                (pos[0] - self.imageSize // 2, pos[1] - self.imageSize // 2),
            )
    ...
    class WaterSpirit(Ally):
        ...
        def update(self):
            ...
            if self.stateTimer >= 90:
                game.room.projectiles.append(
                    Sapphire(self.x, self.y, self.xdir * 3, self.ydir * 3)
                )
                self.stateTimer = 0
    ...
    class FireSpirit(Ally):
        ...
        imageSize = 128
        images = [
            loadTexture("allies/fire.png", imageSize, mirror=True),
            loadTexture("allies/fire2.png", imageSize, mirror=True),
        ]
        ...
        def update(self):
            if random.random() < 0.5:
                ...
                hyp = math.sqrt(
                    (game.player.x - self.x) ** 2 + (game.player.y - self.y) ** 2
                )
                if hyp > 0:
        ...
        def draw(self):
            ...
            pos2 = (
                int((display[0] - game.room.roomSize[0]) / 2 + game.player.x),
                int((display[1] - game.room.roomSize[1]) / 2 + game.player.y),
            )
    ...
    class Enemy:
        ...
        def die(self):
            ...
            for i in range(self.coins):
                game.room.items.append(
                    Coin(self.x + random.randint(-8, 8), self.y + random.randint(-8, 8))
                )
        ...
        def draw(self):
            ...
            if self.state == -2:
                gameDisplay.blit(
                    self.frozenImage, (int(pos[0]) - 128 // 2, int(pos[1]) - 128 // 2)
                )
            if self.burning > 0:
                gameDisplay.blit(
                    self.burningImage, (int(pos[0]) - 128 // 2, int(pos[1]) - 128 // 2)
            ...
            gameDisplay.blit(
                self.image,
                (int(pos[0]) - self.imageSize // 2, int(pos[1]) - self.imageSize // 2),
            )
            # pygame.draw.circle(gameDisplay, (100,100,200), (self.x, self.y), self.radius)
        ...
        def hurt(self, damage=None, knockback=None, attacker=None):
            ...
            if knockback == None:
                ...
                self.knockbackAngle = math.atan2(
                    self.y - attacker.y, self.x - attacker.x
                )
            if damage > 0.3:
                random.choice(Sound.hitSounds).play()
                pos = (
                    (display[0] - game.room.roomSize[0]) / 2 + self.x - 64,
                    (display[1] - game.room.roomSize[1]) / 2 + self.y - 64,
                )
    ...
    class Pufferfish(Enemy):
        ...
        attackImages = [
            loadTexture("enemies/pufferfish/attack" + str(i) + ".png", Animus.imageSize)
            for i in (1, 2, 3)
        ]
    ...
    class Robot(Enemy):
        ...
        def __init__(self, x, y, chief=True):
            ...
            if chief:
                self.friend = Robot(
                    random.randint(100, game.room.roomSize[0] - 100),
                    random.randint(100, game.room.roomSize[1] - 100),
                    chief=False,
                )
        ...
        def update(self):
            ...
            if self.state == 0:
                ...
                if self.friend and self.friend.state == 0:
                    ...
                    if self.chief:  # unnecessary
                        ...
                        if game.findPlayer(
                            self.x + 10 + (self.friend.x - self.x) * randomPoint,
                            self.y + 10 + (self.friend.y - self.y) * randomPoint,
                            4,
                        ):
                            game.player.hurt()
                else:
            ...
            if self.state == 1:
                ...
                if self.stateTimer == 1:
                    ...
                    hyp = math.sqrt(
                        (self.x - game.player.fakeX) ** 2
                        + (self.y - game.player.fakeY) ** 2
                    )
                    dx = (game.player.fakeX - self.x) / hyp
                    dy = (game.player.fakeY - self.y) / hyp
                    game.room.projectiles.append(
                        Missile(self.x, self.y, dx * 3, dy * 3)
                    )
        ...
        def draw(self):
            ...
            if (
                self.state == 0
                and self.friend
                and self.friend.state == 0
                and self.chief
            ):
                clonepos = (
                    int((display[0] - game.room.roomSize[0]) / 2 + self.friend.x),
                    int((display[1] - game.room.roomSize[1]) / 2 + self.friend.y),
                )
                pygame.draw.line(
                    gameDisplay,
                    (200, 200, 200 + random.random() * 55),
                    (pos[0] + 10, pos[1]),
                    (clonepos[0] + 10, clonepos[1]),
                    random.randint(1, 8),
                )
            super().draw()
    ...
    class SkuggVarg(Enemy):
        ...
        attackImages = [
            loadTexture(
                "enemies/skugg/SkuggVarg_" + str(i + 38) + ".png", Animus.imageSize
            )
            for i in range(5)
        ]
    ...
    class Schmitt(Enemy):
        ...
        attackImages = [
            loadTexture("enemies/axereaper/attack" + str(i) + ".png", 256)
            for i in (1, 2)
        ]
            ...
            if self.state == 1:
                ...
                if self.stateTimer == 1:
                    ...
                    target = game.findPlayer(
                        self.x + self.xdir * 16, self.y + self.xdir * 16, 64
                    )
                    if target:
                        target.hurt()
                elif self.stateTimer == 40:
    ...
    class Mercenary(Enemy):
        ...
        walkImages = [
            loadTexture("player/warrior/walk1.png", imageSize),
            loadTexture("player/warrior/walk2.png", imageSize),
        ]
        attackImages = [
            loadTexture("player/warrior/strike1.png", imageSize),
            loadTexture("player/warrior/strike2.png", imageSize),
        ]
        rollImages = [
            loadTexture("player/warrior/roll1.png", imageSize),
            loadTexture("player/warrior/roll2.png", imageSize),
        ]
    ...
    class fungusArm(Enemy):
        ...
        attackImages = [
            loadTexture("enemies/lord fungus/arm" + str(i) + ".png", 128)
            for i in (1, 2, 3)
        ]
        ...
        def update(self):
            ...
            if self.state == 1:
                ...
                if 60 < self.stateTimer < 150:
                    ...
                    self.transformationCharger += 1
                    self.image = self.idleImage
                    enemy = random.choice([Animus, Pufferfish, Svamp, Skull, Hjuldjur])(
                        self.x, self.y
                    )
    ...
    class Boss(Enemy):
        ...
        def update(self):
            ...
            if self.state == 2:
                ...
                if self.stateTimer in [10, 30, 50]:  # summon spores
                    ...
                    dx = (
                        (game.player.fakeX - self.x + random.randint(-60, 60))
                        * 0.8
                        / 120
                    )
                    dy = (
                        (game.player.fakeY - self.y + random.randint(-60, 60))
                        * 0.8
                        / 120
                    )
            ...
            if self.state == 3:
                ...
                if self.stateTimer == 20:  # summon arms
                    ...
                    arm = fungusArm(
                        game.player.fakeX + random.randint(-16, 16),
                        game.player.fakeY + random.randint(-16, 16),
                        owner=self,
                    )
        ...
        def drawUI(self):
            ...
            pygame.draw.rect(
                ...
                (200, 0, 0),
                (
                    display[0] // 2 - self.maxhp // 2 * scale,
                    display[1] - 50 - 32,
                    scale * self.hp,
                    32,
                ),
        ...
        def update(self):
            ...
            if self.state == 2:
                ...
                if self.stateTimer < 20:  # prebite
                    ...
                    displacement = -40
                    target = game.findPlayer(
                        self.x + math.cos(self.a) * displacement,
                        self.y + math.sin(self.a) * displacement,
                        32,
                    )
                    if target:
                        target.hurt()
                ...
                #   Sound.horrorSound.play()
                    ...
                    if random.random() < 0.3:
                        ...
                        proj = Firering(
                            self.x + math.cos(self.a) * displacement,
                            self.y + math.sin(self.a) * displacement,
                            -math.cos(self.a + random.randint(-2, 2) * 0.2) * 2,
                            -math.sin(self.a + random.randint(-2, 2) * 0.2) * 2,
                        )
    ...
    class Projectile:
        ...
        def update(self):
            ...
            if not self.evil and False:
                if game.player.magnet:
                    targets = game.findEnemies(
                        self.x, self.y, 50 + 20 * game.player.magnet
                    )
                    if targets:
                        target = random.choice(targets)
                        hyp = math.sqrt(
        ...
        def draw(self):
            ...
            if game.player.magicWand and not self.evil:
                gameDisplay.blit(
                    self.magicglowImage, (int(pos[0]) - 64 // 2, int(pos[1]) - 64 // 2)
                )
            if self.drawAngled:
                blitRotate(
                    gameDisplay,
                    ...
                    (int(pos[0]), int(pos[1])),
                    (self.imageSize // 2, self.imageSize // 2),
                    math.atan2(-self.yv, -self.xv),
                )
            else:
                gameDisplay.blit(
                    self.image,
                    (
                        int(pos[0]) - self.imageSize // 2,
                        int(pos[1]) - self.imageSize // 2,
                    ),
                )
        ...
        def draw(self):
            ...
            if self.shopItem:
                textsurface = myfont.render(
                    str(self.price) + " coins", False, (0, 0, 0)
                )
    ...
    class StairCase(Item):
        ...
        def pickup(self):
            ...
            for i in range(min(50, game.player.coins * game.player.piggyBank // 2)):
                game.room.items.append(
                    Coin(random.randint(100, 400), random.randint(100, 400))
                )
        ...
        def pickup(self):
            ...
            if game.player.mosscrystal:
                for i in range(game.player.mosscrystal):
                    ...
                    dy = math.sin(a)
                    game.room.projectiles.append(
                        Emerald(self.x, self.y, dx * 3, dy * 3)
                    )

        def update(self):
            super().update()
            if game.player.magnet and game.findPlayer(
                self.x, self.y, 100 * game.player.magnet
            ):
                target = game.player
    ...
    roomPresets=[
        ...
        [
            ...
            [
                createF(
                    [Robot],
                    lambda: random.randint(100, game.room.roomSize[0] - 100),
                    lambda: random.randint(100, game.room.roomSize[1] - 100),
                ),
                createF(
                    [Svamp],
                    lambda: random.randint(100, game.room.roomSize[0] - 100),
                ...
                createF(
                    [Sledger],
                    lambda: random.randint(200, game.room.roomSize[0] - 200),
                    lambda: random.randint(200, game.room.roomSize[1] - 200),
                    depth=3,
                ),
                createF(
                    [Tnt],
        ...
        [
            [
                createWallF(
                    lambda: random.randint(100, 400),
                    lambda: random.randint(100, 400),
                    lambda: random.randint(20, 100),
                    lambda: random.randint(20, 200),
                    occurance=0.5,
                ),
                createWallF(
                    lambda: random.randint(100, 400),
        ...
        [
            [
                createWallF(
                    lambda: random.randint(1, 4) * 100,
                    lambda: random.randint(1, 3) * 100 + 50,
                    20,
                    120,
                ),
            ]
        ...
        [
            ...
            * 4
            + [
                createF(
                    [Svamp, Hjuldjurplant, BlazeSpitter],
                    lambda: random.randint(100, game.room.roomSize[0] - 100),
                    lambda: random.randint(100, game.room.roomSize[1] - 100),
                    depth=2,
                ),
    ...
    class CharacterSelect:
        ...
        def draw(self):
            ...
            for i in range(len(allClasses)):
                ...
                if i == self.cursor:
                    pygame.draw.rect(
                        ...
                        (120, 100, 50),
                        (
                            allClasses_draw_anchor[0] + i * 120,
                            allClasses_draw_anchor[1],
                            128,
                            128,
                        ),
                    )
                ...
                gameDisplay.blit(
                    img,
                    (allClasses_draw_anchor[0] + i * 120, allClasses_draw_anchor[1]),
                )

            class_draw_anchor = (200, 200)
            selected_class = allClasses[self.cursor]
            pygame.draw.rect(
                gameDisplay, (120, 100, 50), (*class_draw_anchor, 256, 256)
            )
            ...
            gameDisplay.blit(
                classname_text_surf,
                (class_draw_anchor[0] + 20, class_draw_anchor[1] + 256 + 40),
            )
            for row in range(len(info_text)):
                ...
                gameDisplay.blit(
                    info_text_surf,
                    (
                        class_draw_anchor[0] + 256 + 40,
                        class_draw_anchor[1] + 20 + 22 * row,
                    ),
                )
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN and not Game.playing_a_run:
                if event.key in [pygame.K_RIGHT, pygame.K_d]:
                    characterSelect.cursor = min(
                        characterSelect.cursor + 1, len(allClasses) - 1
                    )
